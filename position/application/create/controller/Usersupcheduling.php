...
use think\Db;
class Usersupcheduling extends Controller{
    ...
    private function scheduling(){
        ...
        if(!empty($rule_id)){
            ...
            $this->day_data($month_days, $rule, $data);
            $this->day_data($next_month_days, $rule, $data);
        }
    }
    /*
     * 排班
    ...
    function day_data($days,$rule,$data){
        ...
        foreach ($days as $day) {
            ...
            $map['month'] = $m;
            $map['day'] = $d;
            $map['emp_no'] = $user['emp_no'];
            $scheduling_data = Db::name('pos_scheduling')->field('id')->where($map)->find();
            $user_scheduling[$day]['id'] = $scheduling_data['id'];
            Db::name('pos_scheduling')->update($user_scheduling[$day]);
        }
        return "OK";
    }
