...
class RoleController extends HomeController {
	...
	public function index() {
		...
		//全部权限
		$Topnode=M('node')->where('pid=0')->select();
	...
	public function node() {
		...
		$roleRes=$role_user
				->alias('A')
				->field('*')
				->join('LEFT JOIN think_role AS B ON A.role_id=B.id and B.node_id is not null')
				-> where(array('user_id'=>$_SESSION['auth_id']))
...
//
		...
		if(empty($roleRes[0]['node_id'])){
		$group_list = $node_model -> where('pid=0') -> order('sort asc') -> getField('id,name');
		}else{
		$group_list = $node_model -> where("pid=0 and id={$roleRes[0]['node_id']}") -> order('sort asc') -> getField('id,name');
		}
...
//		$user_list = D("UserView") -> where($map)->select();
                $keyword = I('keyword');
		if (!empty($keyword)) {
			$map['_string'] = "u.emp_no like '%$keyword%' OR u.name like '%$keyword%'";
                        $this->assign('keyword',$keyword);
		}
                $user_list = D("User")->get_user_list_limit($map);
