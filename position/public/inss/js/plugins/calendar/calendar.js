...
//国历节日 *表示放假日
var sFtv = new Array("0101*元旦节", "0214 情人节", "0308 妇女节", "0401 愚人节", "0501*劳动节", "0504 青年节", "0601 国际儿童节", "0701中共诞辰", "0801 建军节", "0910 中国教师节", "1001*国庆节", "1224 平安夜", "1225 圣诞节");

//农历节日 *表示放假日
var lFtv = new Array("0101*春节", "0115 元宵节", "0505*端午节", "0707 七夕情人节", "0715 中元节", "0815*中秋节", "0909 重阳节", "1208 腊八节", "1223 小年", "0100*除夕");

//某月的第几个星期几
var wFtv = new Array("0520 国际母亲节", "0630 父亲节", "1144 感恩节");
...
function calendar(y, m) {
	...
	sDObj = new Date(y, m, i + 1 - this.firstWeek);
	//当月一日日期
	...
	if (m2 < 2)
		cY = cyclical(y2 - 1900 + 36 - 1);
	else
		cY = cyclical(y2 - 1900 + 36);
	var term2 = sTerm(y2, 2);
	//立春日期

	////////月柱 1900年1月小寒以前为 丙子月(60进制12)
	var firstNode = sTerm(y2, m2 * 2);
	//返回当月「节」为几日开始
	...
	for (var i = 0; i < 42; i++) {
		...
		//cYear,cMonth,cDay
		this[i] = new calElement(y2, m2 + 1, d2, nStr1[(i + this.firstWeek) % 7], lY, lM, lD++, lL, cY, cM, cD);
		//公历节日
		for (x in sFtv)
		if (sFtv[x].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
			if (Number(RegExp.$1) == (m2 + 1)) {
				if (d2 == RegExp.$2) {
	...
	if (wFtv[i].match(/^(\d{2})(\d)(\d)([\s\*])(.+)$/))
		if (Number(RegExp.$1) == (m + 1)) {
			...
			if (tmp1 < 5)
				this[((this.firstWeek > tmp2) ? 7 : 0) + 7 * (tmp1 - 1) + tmp2].solarFestival += RegExp.$5 + ' ';
			else {
				tmp1 -= 5;
	...
	//今日
	if (y == tY && m == tM)
		this[tD + this.firstWeek - 1].isToday = true;
}
	...
	for ( i = 0; i < 42; i++) {
		...
		if (true) {//日期内
			...
			if (cld[sD].isToday && sObj.parentNode.className.indexOf("todayColor") == -1) {
				sObj.parentNode.className = sObj.parentNode.className + ' todayColor';
				//今日颜色
			}
			m = "0" + cld[sD].sMonth;
			d = "0" + cld[sD].sDay;
