...
class FlowController extends HomeController {
	...
	function index() {
		...
		$role_list = D("Role") -> get_role_list($user_id);//角色id
...
//                                    $map['step'] = array('egt', 0);
                        ...
                        case 'refer':
                            ...
                            break;
                        case 'suggestion':
                            $this -> assign("folder_name", '单据意见');
                            $FlowSuggestion = M('flow_suggestion');
                            $where['emp_no'] = $emp_no;
                            $where['_string'] = "comment is null";
                            $log_list = $FlowSuggestion -> where($where) -> field('flow_id') -> select();
                            $log_list = rotate($log_list);
                            if (!empty($log_list)) {
                                    $map['id'] = array('in', $log_list['flow_id']);
                            } else {
	...
	function folder($fid) {
		...
		$emp_no = get_emp_no(); //获取当前用户
		$user_id = get_user_id(); //当前用户ID
		
		$flow_type_where['is_del'] = array('eq', 0);
		//所有的流程类型
		$flow_type_list = M("FlowType") -> where($flow_type_where) -> getField("id,name"); //全部fa'q项目名称
		...
		// dump($this -> _flow_auth_filter($folder, $map));
                
		$model = D("FlowView");
		if (I('mode') == 'export') {
			$this -> _folder_export($model, $map);
		} else {  
			$this -> _listw($model, $map, 'id desc');
		}
		$this -> display();
	}
	...
	private function _folder_export($model, $map) {
		...
		if ($r <= 1000) {
			...
			$objPHPExcel -> getProperties() -> setCreator("中致远OA") -> setLastModifiedBy("中致远OA") -> setTitle("Office 2007 XLSX Test Document") -> setSubject("Office 2007 XLSX Test Document") -> setDescription("Test document for Office 2007 XLSX, generated using PHP classes.") -> setKeywords("office 2007 openxml php") -> setCategory("Test result file");
			// Add some data
			$i = 1;
			//dump($list);
	...
	function add() {
		...
		$this -> assign("flow_type", $flow_type);
                //请假、公出、出差
                if($flow_type['type'] == 4 || $flow_type['type'] == 5 || $flow_type['type'] == 6){
                    $map['emp_no'] = get_emp_no();
                    $map['schedual_id'] = ['gt',0];
                    $map['year'] = date('Y');
                    $map['month'] = (int)date('m');
                    $scheduling = M('pos_scheduling')->where($map)->find();
                    $schedual = M('pos_schedual')->find($scheduling['schedual_id']);
                    $start_time_arr = explode(':', $schedual['work_start_time']);
                    $end_time_arr = explode(':', $schedual['work_end_time']);
                    $this->assign([
                        'start_time_hour'=>$start_time_arr[0],
                        'start_time_min'=>$start_time_arr[1],
                        'end_time_hour'=>$end_time_arr[0],
                        'end_time_min'=>$end_time_arr[1],
                    ]);
                }
	...
	function read($id) {
		...
		$fid = I("fid");
                //表单令牌
                if($fid == 'confirm' && empty(session('token'))){
		...
		if (empty($vo)) {
                        $this->success('您已提交过建议请勿重复提交',U('folder',array('fid'=>'suggestion')));
		}
		...
		$where['step'] = array('lt', 100);
                $where['_string']="result<>9 OR result IS NULL" ;
               	$where['is_del'] = 0;

		$flow_log = $model
                        ->where($where) -> order("id") -> select();
                $suggest_m = M('flow_suggestion');
                $where_s = array();
                foreach ($flow_log as $k => $v){
                    $where_s['flow_log_id'] = $v['id'];
                    $suggest = $suggest_m->field('id,user_name,comment,write_time,question')->where($where_s)->select();
                    $flow_log[$k]['suggest'] = $suggest;
                }
		...
		$this -> assign("refer_flow_log", $refer_flow_log);		
                //收集建议
                if($fid == 'suggestion'){
                    $flow_suggestion_m = M('flow_suggestion');
                    $where = array();
                    $where['flow_id'] = $id;
                    $where['emp_no'] = get_emp_no();
                    $where['_string'] = "comment is null";
                    $to_suggestion = $flow_suggestion_m -> where($where) -> find();                        
                    $this -> assign("to_suggestion", $to_suggestion);
                }
                //当前审批信息
                if($fid == 'confirm'){
                    $where = array();
	...
	protected function _insert($name = CONTROLLER_NAME) {
		...
		$str_confirm = D("Flow") -> _conv_auditor($model -> confirm); //审批流程
		
                $model->refer_name = I('refer_name');
                $str_refer = D("Flow") -> _conv_auditor($model -> refer);				                
                
		$str_auditor = $str_confirm;
		...
		$model -> udf_data = D('UdfField') -> get_field_data();
 		//日志审批流
 		writerLogs($_SESSION['emp_no']."(".$_SESSION['user_name'].")".$str_confirm,'log','flow');
 		$model -> pos_type = I('pos_type');
	...
	protected function _assign_tag_list() {
        ...
        protected function _assign_tag_list_arr($arr) {
		$model = D("SystemTag");
		$tag_list = $model -> get_tag_list('id,name', 'FlowType',$arr);
		$this -> assign("tag_list", $tag_list);
	}
        ...
        function dept_list(){
            ...
            $map['is_del'] = 0;
            $dept_list = M('dept')->where($map)->select();
            return $dept_list;
        }
}
