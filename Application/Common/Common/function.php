...
/*---------------------------------------------------------------------------
 ...
 -------------------------------------------------------------------------*/
require_once './Application/Common/Common/JSSDK.php';
require_once './Application/Common/Common/redirect.php';
require_once './Application/Common/Common/wx/Material.php';
require_once './Application/Common/Common/wx/SendMessage.php';

/*
 * 写日志
 * param $log 日志内容
 * param $type 日志类型，默认log
 * param $path 日志路径
 */
function writerLogs($log, $type='log', $path) {
    vendor('Log.Log');
    Log::writer($log, $type, $path);
}
/**
 * 系统加密方法
...
function tp_redis($name,$value="",$time=""){
    $redis = new \Redis();
    $redis->connect(C('redis_url'), C('redis_port'));
 ...
 * 获取公司id
 */
//function get_com_id($emp_no = null){
//    if (empty($com_id)) {
//		$result = M("user_com") -> find(session("emp_no"));
//		return $result['name'];
//	} else {
//		$where['id'] = array('eq', $com_id);
//		return M("Dept") -> where($where) -> getField('name');
//	}
//}
...
function organ_tree_menu($tree,$level=0){
		$level++;
	$html = "";
	if (is_array($tree)) {
		$html = "<ul class=\"tree_menu\">\r\n";
		foreach ($tree as $val) {
			if (isset($val["name"])) {
				...
				if (isset($val['_child'])) {
					...
					$html.="<i class=\"fa fa-angle-right level$level\"></i><span>$title</span></a>\r\n";
					$html = $html . organ_tree_menu($val['_child'], $level);
					$html = $html . "</li>\r\n";
				} else {
					$html = $html . "<li>\r\n<a  node=\"$id\" >";
					for ($i=0; $i <= $level; $i++) { 
...
function send_push($data, $user_list, $time = null, $type = 'text') {
	...
	if (is_array($user_list)) {
        ...
        foreach ($user_list as $v){
            $user = $user_m->field('emp_no')->find($v);
            $url = "https://".$_SERVER['SERVER_NAME']."/index.php?m=&c=Public&a=wxlogin&biao=".$data['biao']."&danju_id=".$data['flow_id'];
             Pushmessage($data['title'],$data['action'].'('.$data['type'].')',$url,$user['emp_no']);
        }
 ...
 * 同时发送多条消息
 */
function send_pushs($data, $user_list, $time = null, $type = 'text') {
	$model = M("Push");
	if (empty($time)) {
		$model -> time = time();
	} else {
	...
	if (is_array($user_list)) {
        ...
        $user = $user_m->field('emp_no')->where($user_map)->select();
        $user_arr=array();
        foreach ($user as $k => $v) {
            $user_arr[$k]= $v['emp_no'];
        }
        $url = "https://".$_SERVER['SERVER_NAME']."/index.php?m=&c=Public&a=wxlogin&biao=".$data['biao']."&danju_id=".$data['id'];
        if(count($user_arr)>1000){
            $user_arr2 = array_chunk($user_arr,1000,FALSE);
            foreach ($user_arr2 as $v) {
                Pushmessage($data['title'],$data['action'].'('.$data['type'].')',$url,$v);
            }
        }else{
             Pushmessage($data['title'],$data['action'].'('.$data['type'].')',$url,$user_arr);
        }
}
function send_ws($data, $user_list) {
	...
	$push_api_url = "https://127.0.0.1:2121/";
...
function send_sms($data, $user_list) {
	...
	// $url = 'https://192.168.100.9:9080/OpenMasService?WSDL';
	// $message = $msg;
	// $extendCode = "26";
	//
	// //自定义扩展代码（模块）
	...
	$url = "https://sh2.ipyy.com/sms.aspx?action=send&userid=&account={$sms_user}&password={$sms_password}&mobile={$sms_user_list}&content={$msg}&sendTime=&extno=";
...
function conv_baidu_map(&$lat, &$lng) {
	$url = "https://api.map.baidu.com/geoconv/v1/?coords=$lng,$lat&from=1&to=5&ak=EE6745c36d96321e90b7015f3de4a4ee";
...
function get_location($lat, $lng) {
	...
	$url = "https://api.map.baidu.com/geocoder/v2/?ak=EE6745c36d96321e90b7015f3de4a4ee&callback=renderReverse&location=$lng,$lat&output=json";
...
function tp_getAccess_Token() {
    $access_token = tp_redis('access_token');
    if(empty($access_token)){
        ...
        tp_redis('access_token',$data['access_token'],6800);
...
/*推送消息*/
 function Pushmessage($title,$description,$url,$usermess){
     ...
     return $sendMessage->sendNews(C('appid'), array(array('title'=>$title, 'description'=>$description, 'url'=>$url)),$usermess);
 }